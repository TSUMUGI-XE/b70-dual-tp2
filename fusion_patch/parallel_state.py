...
class GroupCoordinator:
    ...
    def __init__(
        ...
        for ranks in group_ranks:
            # [B70 TP=2 fusion] oneCCL/xccl OFI-ATL handshake hangs on B70+xe for
            # same-root-complex P2P (#41663/#8022): new_group(backend="xccl") never
            # returns. When VLLM_XPU_HOSTSTAGED_TP=1, create the device group on gloo
            # instead (no xccl ever) and route GPU collectives through the host
            # (see XpuCommunicator). env off => identical upstream behavior.
            import os as _os
            _hoststaged_tp = _os.environ.get("VLLM_XPU_HOSTSTAGED_TP", "0") == "1"
            _dev_backend = "gloo" if _hoststaged_tp else torch_distributed_backend
            device_group = torch.distributed.new_group(
                ranks, backend=_dev_backend
            )
...
def init_distributed_environment(
    ...
    if not torch.distributed.is_initialized():
        ...
        if not torch.distributed.is_backend_available(backend):
            ...
            backend = "gloo"
        # [B70 TP=2 fusion] force the WORLD backend to gloo too, so no xccl process
        # group is ever created (the xccl OFI/ATL handshake hangs on B70+xe same-RC).
        import os as _os
        if _os.environ.get("VLLM_XPU_HOSTSTAGED_TP", "0") == "1":
            backend = "gloo"
